...
public SqlRepository(string connectionString)
{
    this._connectionString = connectionString ?? throw new ArgumentNullException(nameof(connectionString));
        ...
        public List<Product> GetAllProducts(int location_id_input)
        {
            ...
            using SqlCommand cmd = new($"SELECT * FROM Product WHERE location_id = {location_id_input}", connection);
            ...
            while (reader.Read())
            {
                ...
                result.Add(new Product (id, name, amount, price, description, location_id, CreatedAt, UpdatedAt));
            }
        ...
        public Guest ViewGuest(int id, int location_id)
        {
            ...
            while(reader.Read())
            {
                ...
                int Product_id = reader.GetInt32(11);
                Guest guest = new Guest(id, first_name, last_name, Product, location_id, Product_id, created_at, updated_at, checked_in, durationofstay, credit, confirmation_number);
                return guest;                
            }
        ...
        public bool RegisterGuest(string confirmation_number, string first_name, string last_name, string Product, decimal credit, int durationofstay, byte checked_in, int location_id, int Product_id)
        {
            ...
            if(confirm_result == "")
            {
                ...
                connection.Open();
                using SqlCommand cmd2 = new SqlCommand($"SELECT amount, price FROM Product WHERE id = {Product_id} AND location_id = {location_id}", connection);
                ...
                else
                {
                    ...
                    connection.Open();
                    using SqlCommand cmd3 = new($"INSERT INTO GUEST (first_name, last_name, Product, credit, confirmation_number, durationofstay, checked_in, location_id, Product_id) VALUES ('{first_name}', '{last_name}', '{Product}', {(Convert.ToDecimal(credit-(price_result*durationofstay)))}, '{confirmation_number}', {durationofstay}, {checked_in}, {location_id}, {Product_id});", connection);
                    using SqlDataReader reader3 = cmd3.ExecuteReader();
                    connection.Close();
                    connection.Open();
                    using SqlCommand cmd4 = new SqlCommand($"UPDATE Product SET amount = {amount_result-1}, updated_at = {systime} WHERE id = {Product_id} AND location_id = {location_id};", connection);
    ...
    public List<Guest> GuestLookUp(string input, int location_id)
    {
        ...
        byte t = 1;
        byte f = 0;
        using SqlCommand cmd = new($"SELECT * FROM Guest WHERE (Product = '{input}' AND location_id = {location_id}) OR (first_name = '{input}' AND location_id = {location_id}) OR (last_name = '{input}' AND location_id = {location_id}) OR (confirmation_number = '{input}' AND location_id = {location_id});", connection);
        using SqlDataReader reader = cmd.ExecuteReader();
        while(reader.Read())
        {
            int id = reader.GetInt32(0);
            ...
            int Product_id = reader.GetInt32(11);
            result.Add(new Guest (id, first_name, last_name, Product, Location_id, Product_id, CreatedAt, UpdatedAt, checked_in, durationofstay, credit, confirmation_number));
        }
        return result;
    }
    ...
    public List<Guest> GuestAll(int location_id)
    {
        ...
        public void EditGuest(int id, string confirmation_number, string first_name, string last_name, string Product, decimal credit, int durationofstay, int location_id, int Product_id)
        {
        using SqlConnection connection = new SqlConnection(this._connectionString);
        connection.Open();
        using SqlCommand cmd = new SqlCommand($"UPDATE Guest SET first_name = '{first_name}', last_name = '{last_name}', Product = '{Product}', credit = {credit}, durationofstay = {durationofstay}, updated_at = SYSDATETIME(), Product_id = {Product_id} WHERE id = {id}", connection);
        using SqlDataReader reader = cmd.ExecuteReader();
    }
